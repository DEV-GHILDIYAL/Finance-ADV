...
router.post("/setpassword", async (req, res) => {
  ...
  try {
    ...
    const secret = process.env.JWT_SECRET_KEY + user._id;
    const ltoken = jwt.sign({ email: user.email, id: user._id }, secret, { expiresIn: "5m" });
    const link = `${process.env.FRONT_URL}/api/resetpassword/${user.email}/${ltoken}`;
...
// Verify Token and Return Status
router.get("/resetpassword/:email/:ltoken", async (req, res) => {
  const { email, ltoken } = req.params;
...
// Set New Password
router.post('/resetpassword/:email/:ltoken', async (req, res) => {
  const { email, ltoken } = req.params;
  const { password } = req.body;
